...
st.set_page_config(
    page_title="INOVALENIN - Dashboard Financeiro v8.0.1",
    page_icon="📊",
    layout="wide",
...
# ==============================================================================
# LÓGICA DO DASHBOARD (VERSÃO 8.0.1 - UNIFICADA)
# ==============================================================================
...
class AnalistaFinanceiro:
    ...
    def calcular_kpis(self):
        ...
        return {
            # --- Índices de Liquidez & Solvência (v7.9) ---
            ...
            # --- Índices de Rentabilidade & Performance (v8.0) ---
    ...
    def gerar_score(self, kpis):
        score = 50
        # Regras de Liquidez (Peso alto)
        if kpis["Liquidez Corrente"] >= 1.0: score += 15
        if kpis["Liquidez Geral"] >= 1.0: score += 10
...
def consultar_ia_financeira(api_key, modelo_escolhido, kpis, dados_dre, nome_empresa, cnpj_empresa, periodo_analise):
    ...
    # --- PROMPT V8.0.1 (FUSÃO DE CONTEXTOS) ---
    prompt = f"""
    {contexto}
    ...
    DADOS APURADOS - BALANÇO (Liquidez e Solvência):
    ...
    DADOS APURADOS - DRE (Performance Operacional):
    - Receita Bruta: R$ {dados_dre.receita_bruta:,.2f}
    - Receita Líquida: R$ {dados_dre.receita_liquida:,.2f}
    - Lucro Bruto: R$ {dados_dre.lucro_bruto:,.2f} (Margem Bruta: {kpis['Margem Bruta (%)']:.1f}%)
    - Resultado Operacional: R$ {dados_dre.resultado_operacional:,.2f} (Margem Operacional: {kpis['Margem Operacional (%)']:.1f}%)
    - Resultado Líquido: R$ {dados_dre.lucro_liquido:,.2f} (Margem Líquida: {kpis['Margem Líquida (%)']:.1f}%)
    - GAO (Alavancagem Operacional): {kpis['GAO (Alavancagem)']:.2f}
    ...
    [Analise a Liquidez Corrente, Seca e Geral. A empresa paga suas contas no curto e longo prazo? Há dependência de estoques?]
    
    ## 2.2 Estrutura de Capital (Endividamento)
    [Analise o Endividamento Geral. O capital é próprio ou de terceiros? O risco de insolvência é alto?]

    # 3. Análise de Performance e Rentabilidade (DRE)
    ## 3.1 Eficiência Operacional
    [Analise a Margem Bruta e Operacional. Os custos e despesas estão controlados em relação à receita?]
    
    ## 3.2 Resultado Líquido e Retorno
    [Analise a Margem Líquida final. A operação vale a pena? O GAO indica alto risco de alavancagem?]

    # 4. Conclusão Técnica e Recomendações
    [Parecer final sintético unindo Balanço e DRE]
    ## 4.1 Plano de Ação Imediato
    [3 sugestões práticas para melhorar os indicadores críticos encontrados]
...
def main():
    ...
    st.title("Dashboard Analista Balanço (v 8.0.1)")
    ...
    if check_zeros:
        st.warning("⚠️ Campos da DRE zerados. Edite abaixo para análise completa.")
    ...
    with st.expander("📝 Editar/Corrigir Valores Extraídos (Clique para abrir)", expanded=check_zeros):
        ...
        with c3:
            ...
            dre.resultado_operacional = st.number_input("Resultado Operacional", value=dre.resultado_operacional, format="%.2f")
    ...
    d4.metric("GAO (Alavancagem)", f"{kpis['GAO (Alavancagem)']:.2f}")
    d5.metric("Peso Desp. Oper.", f"{kpis['Índice Desp. Operacionais (%)']:.1f}%")

    with st.expander("📐 Ver Fórmulas e Notas Explicativas"):
        st.markdown("""
        * **Liquidez Geral:** $\\frac{\\text{AC} + \\text{ANC}}{\\text{Passivo Total Exigível}}$
        * **Margem Bruta:** Eficiência da produção/aquisição.
        * **GAO:** Sensibilidade do lucro em relação à variação das vendas.
        """)

    st.divider()
    st.subheader("📝 Relatório de Análise Financeira")
    if st.button("✨ Gerar Análise Automatizada (v8.0.1)", type="primary"):
